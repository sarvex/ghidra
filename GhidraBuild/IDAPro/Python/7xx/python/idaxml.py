...
class XmlExporter(IdaXml):
    ...
    def export_stack_vars(self, function, sframe):
        ...
        for i in range(sframe.memqty):
            ...
            if regcmt != None:
                regcmt = ida_lines.tag_remove(f"{regcmt} ", 0)
            if rptcmt != None:
                rptrcmt = ida_lines.tag_remove(f"{rptcmt} ", 0)
    ...
    def import_xml(self):
        ...
        for event,element in cElementTree.iterparse(self.filename,
            ...
            if self.debug == True and event == 'start':
                msg = ''
                if element.tag != None:
                    msg += f'{str(element.tag)} '
                if element.attrib != None:
                    msg += f'{str(element.attrib)} '
                if element.text != None:
                    msg += str(element.text)
                if msg != "":
                    idc.msg('\n' + msg)

            if event in self.callbacks and element.tag in self.callbacks[event]:
                if event == 'start':
